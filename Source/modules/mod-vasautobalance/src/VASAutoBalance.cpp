...
### Description ###
------------------------------------------------------------------------------------------------------------------
This script is intended to scale based on number of players, instance mobs & world bosses' level, health,
mana, and damage.
...
### Version ###
------------------------------------------------------------------------------------------------------------------
- v2018.12.12 - Update module for StygianCore
- v2017.08.23 - Update module announce, Update strings
-
...
#### Additional Credits
- [CVMagic](http://www.trinitycore.org/f/topic/6551-vas-autobalance/)
- [ScriptDev2](https://scriptdev2.svn.sourceforge.net/)
- [VAS KalCorp/Vaughner](http://vasserver.dyndns.org/)
...
public:
    ...
    void OnBeforeConfigLoad(bool reload) override
    {
        ...
        if (!reload) {
            ...
            std::string cfg_file = conf_path + "/mod_vasutobalance.conf";
#ifdef WIN32
            cfg_file = "mod_vasautobalance.conf";
#endif
    ...
    void SetInitialWorldSettings()
    {
        ...
        announce = sConfigMgr->GetIntDefault("VASAutoBalance.announce", 1) == 1;
    ...
    void OnLogin(Player *Player)
    {
        if (enabled)
        {
            if (announce)
            {
                ...
                //ChatHandler(Player->GetSession()).PSendSysMessage("This server is running the |cff4CFF00VASAutoBalance |rmodule.");
            }
        }
    }
